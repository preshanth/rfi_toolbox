...
class RFIMaskDataset(Dataset):
    ...
    def _normalize_input(self, input_np):
         if self.normalization == 'global_min_max':
            if self.global_max > self.global_min:
                return (input_np - self.global_min) / (self.global_max - self.global_min)
            else:
    ...
    def _generate_ms_samples(self):
        ...
        for row_num in tqdm(range(num_rows), desc="Processing MS Rows"):
            ...
            input_data = np.stack([
                combined_data[0].real, combined_data[0].imag,
                combined_data[1].real, combined_data[1].imag,
                combined_data[2].real, combined_data[2].imag,
                combined_data[3].real, combined_data[3].imag
            ], axis=0)  # shape: (8, freq_bins, time_bins)
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Generate or load RFI dataset as numpy files.")
    parser.add_argument("--samples_training", type=int, default=1000, help="Number of training samples.")
    parser.add_argument("--samples_validation", type=int, default=200, help="Number of validation samples.")
    parser.add_argument("--output_dir", type=str, default="rfi_dataset", help="Output directory.")
    ...
    parser.add_argument("--val_field", type=int, help="FIELD_ID to use for validation set.")
    ...
    if args.use_ms:
        ...
        train_dataset = RFIMaskDataset(data_dir=ms_output_dir, use_ms=True, ms_name=args.ms_name, field_selection=args.train_field)
        val_dataset = RFIMaskDataset(data_dir=ms_output_dir, use_ms=True, ms_name=args.ms_name, field_selection=args.val_field)
